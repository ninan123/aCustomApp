...
@interface GeneralAssViewController ()<UITableViewDataSource,UITableViewDelegate,ZSYPopoverListDatasource,ZSYPopoverListDelegate>
...
@implementation GeneralAssViewController
...
{
    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 67)];
    ...
    label.text = @"分类";
    label.textColor = RGBColor(0, 104, 179);
    label.font = [UIFont boldSystemFontOfSize:16];
    [whiteView addSubview:label];
    
    UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
    button.frame = CGRectMake(label.right, 30, SCREEN_WIDTH-80, 32);
    [button setTitle:@"全部动态" forState:UIControlStateNormal];
    button.titleLabel.font = [UIFont boldSystemFontOfSize:16];
    [button setTitleColor:RGBColor(0, 104, 179) forState:UIControlStateNormal];
    ...
    UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(button.width-31, 5, 25, 25)];
    img.image = [UIImage imageNamed:@"triangleyes"];
    [button addSubview:img];
    //    img.centerY = button.centerY;
    return bgView;
...
{
    return [self setTableHearderView];
}
